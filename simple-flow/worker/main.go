...
func run(c client.Client) {
	...
	w.RegisterWorkflow(simpleflow.ComplexWorkflow)
...
func startSignal(c client.Client, q string) {
	wfid := fmt.Sprintf("mleow-complex")
	...
	wfr, xerr := c.ExecuteWorkflow(context.Background(),
		...
		}, simpleflow.ComplexWorkflow,
		simpleflow.ComplexWFInput{Name: "foo"})
	if xerr != nil {
		// check common cases ...
		if temporal.IsApplicationError(xerr) {
			fmt.Println("xxxx APPERR xxxx")
	...
	var res simpleflow.ComplexWFOutput
	werr := wfr.Get(context.Background(), &res)
	if werr != nil {
		// check common cases ...
		if temporal.IsApplicationError(werr) {
		...
		fmt.Println("DIE!!! in Get of ComplexWorkflow")
		//spew.Dump(werr)
		//panic(werr)
		return
	}
	// Print result of Workflow
	fmt.Println("COMPLEX_RES: ", res)
}
